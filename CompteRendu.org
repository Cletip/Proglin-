...
Pour tester les matrices test, il suffit de donner à A une taille n*n, puis de la remplir, grâce au programme, d'une des matrices test. Le temps d'exécution de la résolution sera donné, ainsi que la solution X.
...
Comme on peut le voir sur les différents jeux d'essais, Jacobi est plus efficace en temps que Gauss, car elle est de complixité coût de l'ordre de $3n^2+2n$ par itération.
Gauss complixité = $2\frac{n^3}{3}$, car $\frac{n^2}{2}$ pour la division et $\frac{n^3}{3}$ pour la multiplication
