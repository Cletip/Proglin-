...
Nous ne mettrons le code des matrices test, mais vous pouvez le retrouver [[file:matricetest.c::{][ici]].


De plus, la plupart des matrices test sont compatibles uniquement avec la méthode de résolution de Gauss. En effet, seule les matrices Km_carré, Bord ainsi que Lehmer seront diagonales dominantes.
...
[[file:resultattestjacogauss.png]]
...
1. Nous voulons avoir une solution rapidement, mais la précision est négligeable à \epsilon près. Nous pouvons donc utiliser la méthode de Jacobi, qui s'effectuera plus rapidement que la méthode de Gauss.
